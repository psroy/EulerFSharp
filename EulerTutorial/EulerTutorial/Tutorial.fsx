...
[ 1 .. 999 ] |> List.filter( fun x -> x % 3 = 0 || x % 5 = 0) |> List.sum |> printfn "Sum : %A"
...
seq{for i in 1..4000000 do yield i} |> Seq.map( fun x -> fib(x)) |> Seq.filter(fun x -> x % 2 = 0) |> Seq.sum |> printfn "Sum : %A"
